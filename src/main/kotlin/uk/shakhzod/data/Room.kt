...
import uk.shakhzod.other.getRandomWords
import uk.shakhzod.other.matchesWord
import uk.shakhzod.other.transformToUnderscores
import uk.shakhzod.other.words
...
class Room(
    ...
    var phase = Phase.WAITING_FOR_PLAYERS
        set(value) {
            synchronized(field) {
                field = value
                phaseChangedListener?.let { change -> change(value) }
            }
        }
    ...
    private suspend fun finishOffDrawing(){
        lastDrawData?.let {
            if(curRoundDrawData.isNotEmpty() && it.motionEvent == 2){ //2 == ACION_MOVE
                val finishDrawData = it.copy(motionEvent = 1) //1 == ACTION_UP
    ...
    suspend fun addPlayer(clientId: String, username: String, socket: WebSocketSession): Player {
        ...
        val player = if(leftPlayers.containsKey(clientId)){
            ...
            leftPlayer?.first?.let {
                //New connection if user left the game and reconnected
                it.socket = socket
    ...
    private fun timeAndNotify(ms: Long) {
        ...
        timerJob = GlobalScope.launch {
            ...
            repeat((ms / UPDATE_TIME_FREQUENCY).toInt()) {
                //We do not need to change the phase everytime..Only the first time is needed and the rest can be null for the phase
                if (it != 0) {
                    phaseChange.phase = null
                }
                broadcast(gson.toJson(phaseChange))
                phaseChange.time -= UPDATE_TIME_FREQUENCY //new remaining time
    ...
    private fun gameRunning() {
        //clearing the state for each round
        winningPlayers = listOf()

        //If countdown runs out but the player does not select a word then select a random word
    ...
    private fun showWord() {
        GlobalScope.launch {
            if (winningPlayers.isEmpty()) {
                drawingPlayer?.let {
                    //If nobody guessed it then it is not drawn properly
    ...
    private fun addWinningPlayer(username: String): Boolean {
        ...
        /*
        If everybody finds the word before the time runs out then start a new round.
        -1 is for excluding the drawing player..
         */
    ...
    companion object {
        ...
        const val GUESS_SCORE_DEFAULT = 50 //For finding
        const val GUESS_SCORE_PERCENTAGE_MULTIPLIER = 50 //This will be based on remaining time
        const val GUESS_SCORE_FOR_DRAWING_PLAYER = 50 //This score is for well-drawing
    }
}